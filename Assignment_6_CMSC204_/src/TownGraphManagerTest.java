...
public class TownGraphManagerTest {
	...
	public void testGetPathB() {
		ArrayList<String> path = graph.getPath(town[1],town[6]);
		  ...
		  //I have manipulate some of the assertEquals statements because there
		  // are typos. For example "Town_10 via Road_8 to Town_9 4 mi", which 
		  // appear in the original Junit file, should be "Town_9 via Road_8 to Town_10 4 mi".
		  // This fact can be verified by looking at how we add Road_8 to the graph
		  // For this reason, I change the statements.
		  assertEquals("Town_1 via Road_2 to Town_3 4 mi",path.get(0).trim());
		  assertEquals("Town_3 via Road_5 to Town_8 2 mi",path.get(1).trim());
		  assertEquals("Town_8 via Road_9 to Town_10 2 mi",path.get(2).trim());
		  assertEquals("Town_9 via Road_8 to Town_10 4 mi",path.get(3).trim());
		  assertEquals("Town_6 via Road_7 to Town_9 3 mi",path.get(4).trim());
